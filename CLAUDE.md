...
every AI instance working in this repo. It holds only the rules that must
never be missed. Everything else lives in `.claude/skills/` and is loaded
by the relevant skill — see `DOCUMENTATION_POLICY.md` for the full system
this belongs to.
...
`CLAUDE.md` §2"). **Never renumber** — it leaves every citation pointing at
...
**Claude never runs a `git` command itself** — not `commit`, `push`,
...
Never add a `Co-Authored-By: Claude` (or similar) trailer to a commit
message, PR description, or code comment, unless explicitly asked for it
...
- `.claude/skills/` — living per-feature docs (architecture, known
  ...
  (dossiers, decisions logs, changelog fragments).
